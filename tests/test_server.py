...
async def create_connection():
    ...
    for i in range(max_retries):
        try:
            ...
            if i < max_retries - 1:
                print(
                    f"[DEBUG] Connection attempt {i + 1} failed ({type(e).__name__}): {e}. Retrying..."
                )
...
class TestRPCOperations:
    ...
    async def test_unauthorized_rpc(self):
        ...
        async with create_connection() as conn:
            result = await conn.send_raw(
                {"method": "__getitem__", "args": ["test"], "kwargs": {}}
            )
            assert isinstance(result, dict)
            assert result.get("status") == "error"
                    ...
                    await conn.send_raw(
                        {
                            "method": "__setitem__",
                            "args": [
                                f"concurrent_test_{client_id}_{i}",
                                {"data": "x" * 100},
                            ],
                            "kwargs": {},
...
class TestSecurity:
    ...
    async def test_invalid_message_format(self):
        ...
        async with create_connection() as conn:
            ...
            # エラーまたは未認証応答が返される
            assert (
                result.get("status") == "error" or "unauthorized" in str(result).lower()
            )
